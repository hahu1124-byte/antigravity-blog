...
| 4 | archive | T版完成に伴い、元データのA版・C版を `archive/` フォルダへ退避 |
| 5 | ep115A | [A版の執筆内容をここに追記] |
| 6 | ep115T | [T版のマージ内容をここに追記] |
| 7 | archive | T版完成に伴い、元データのA版・C版を `archive/` フォルダへ退避 |
| 8 | ep116A | Antigravity IDEで執筆。監察院文書保全課でのシェラ立会いによる照合簿閲覧、001番項目の人為的切除の発見、綴じ糸の結び直しと「該当記録なし」という偽装工作の発覚、アシュが原本に手を伸ばしかけミラに制止される場面、トビアスの不審な挙動という流れで執筆 |
| 9 | ep116C | Claude Code novels-writerサブエージェントが書き下ろしたC版。同様のプロット展開に加え、アシュの鑑定眼《トゥルー・サイト》による欠損枠の物理鑑定シーンを新規構築 |
| 10 | ep116T | A版をベースに、C版の鑑定眼《トゥルー・サイト》シーンをマージしてT版として保存。ファクトチェッカーNLM・編集担当NLM双方のレビューを実施。実在した指摘のみ反映：リューネのセリフ語尾「〜わね」「〜わ」「〜よ」をセイラ口調との混同回避のため留保表現に修正、概数表現「3つ分」を漢数字「三つ分」に統一、既出・定着済みキャラ（ヴィン・シェラ・トビアス）の地の文フルネーム表記を簡略化。両NLM共通の大量のルビ指摘（見張り台・廃地・廃城・玉座の間・東翼・書庫棟等、本文に実在しない語への指摘）、封石2回目ルビ余剰、「現現実」誤字、花霞月ルビのカッコ除去、場面転換前後の空行不足の指摘は本文照合の結果、いずれも本文に実在しない、または既存ルール上正しい記法と判明し誤指摘（幻覚含む）と判定して却下 |
| 11 | archive | T版完成に伴い、元データのA版・C版を `archive/` フォルダへ退避 |
