...
| 3 | setting.md | ep66・68・69のプロットあらすじ末尾演出案に残っていた魔王の代数表記違反（五代目魔王セイラ・三代目魔王リューネ・七代目魔王ライガス）を発見し削除。ep66A.mdの原稿にも同型の代数表記が2箇所あったが、T版マージ時点で既に除去済みだったことを確認。C版執筆エージェントが今後この手の完成文をそのまま転記してしまう構造的リスクを特定 |
| 4 | novels-writer.md / novels-rules3.md | 「執筆ルール＞設定内容＞文脈」の優先順位を明記し、setting.mdのあらすじ完成文をそのまま転記しないルールを追加。自己批評ステップに`grep`による代数表記・暫定継承の機械チェックを追加。起承転結「7話以下」の上限ルールを目安に緩和 |
| 5 | setting.md / EP-timeline.md | 3章（承+1話・転+2話・結+1話、22→26話、ep60〜85）・4章（転+1話、26→27話、ep86〜112）のプロットを再設計。話数枠にイベントを均等配分していたことで重要な戦闘・感情描写が圧縮されていた問題を解消し、ダグ・ヴォルツ戦や古廃甲初対峙などの戦闘を前後編に分割、鎮魂・休息パートを新設 |
